...
# The names of any people you worked with for this assignment: Prashant Toteja
...
#print (api.home_timeline(input('Input Twitter User Handle')))

#search = api.home_timeline(q = input('fff'), rpp = 1)

def get_user_tweets(input_x):
	lst_tweets = []
	#unique_id = "twitter_{}".format(input_x)
	if input_x in CACHE_DICTION:
		lst_tweets = CACHE_DICTION[input_x]
	else:
		search_data = api.user_timeline(input_x)

		string = json.dumps(search_data)
		json_x = json.loads(string)
		lst_tweets = json_x

	CACHE_DICTION[input_x] = lst_tweets
...
statement += "Tweets (tweet_id INTEGER PRIMARY KEY, author TEXT, time_posted TIMESTAMP, tweet_text TEXT, retweets INTEGER)"
...
tweet_by_tweet = list(zip(tweet_id, author, time_posted, tweet_text, retweets))
#print (tweet_by_tweet)
#print (tweet2)

# Use the database connection to commit the changes to the database


committing = 'INSERT INTO Tweets VALUES (?,?,?,?,?)'
	

for x in tweet_by_tweet:
	cur.execute(committing, x)
...
tweet_posted_times = list(cur.execute(statement))
#print (x)
#print (tweet_poster_times)
...
more_than_2_rts = list(cur.execute(statement))
...
rt_lst = list(cur.execute(statement))
#print (rt_lst)

for x in rt_lst:
	#print (x)
	first_rt = list(rt_lst[0])
	# print (first_rt)
	# print (first_rt[0])
	first_rt = first_rt[0]
